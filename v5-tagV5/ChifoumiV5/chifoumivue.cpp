    ...
    , ui(new Ui::ChifoumiVue)
{
    ...
    connect(ui->action_Quitter,SIGNAL(triggered()),this,SLOT(choisirQuitter()));
    connect(ui->actionA_propos_de,SIGNAL(triggered()),this,SLOT(aProposDe()));
    connect(chrono,SIGNAL(timeout()),this,SLOT(topChrono()));
...
void ChifoumiVue::demanderNouvellePartie()
{
    ...
    laPresentation->topChrono();
}

void ChifoumiVue::choisirQuitter()
{
    laPresentation->choisirQuitter();
}
...
void ChifoumiVue::topChrono()
{
    qDebug()<<"Test Hello"<<Qt :: endl;
